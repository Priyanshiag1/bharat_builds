...
def extract_invoice_data(file_bytes: bytes) -> dict:
    ...
    If AWS account is pending backend verification or unreachable, seamlessly extracts
    ...
    try:
        ...
        future = executor.submit(_call_textract)
    ...
    if response and "ExpenseDocuments" in response and len(response["ExpenseDocuments"]) > 0:
